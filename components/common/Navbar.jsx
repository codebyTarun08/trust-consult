    ...
    <div className="sticky top-0 z-50 bg-prime-5 text-gray-400 font-inter font-light border-b-[1px] border-richblack-100/50 backdrop-blur-3xl">
      <nav className="flex items-center justify-between w-11/12 max-w-maxContent py-3 mx-auto">
        ...
        <ul className="hidden md:flex items-center gap-6">
          ...
          <Link href="/about"><li className="hover:text-white transition">About</li></Link>
          <Link href="/contact"><li className="hover:text-white transition">Contact Us</li></Link>
          <Link href="/book">
        ...
        <div className="flex items-center gap-x-4">
                ...
                {token && (
                  <div className="flex items-center gap-x-4">
                    ...
                    <div className="relative">
                      <button aria-label="User menu" onClick={() => setShow((s) => !s)} className="rounded-full overflow-hidden w-10 h-10 border-4 border-green-100/20 shadow-lg">
                        <Image width={100} height={100} src={user?.image} alt="User Avatar" className="w-full h-full object-cover" />
                      </button>
                      {show && (
          ...
          {/* Mobile hamburger */}
          <button className="md:hidden p-2 rounded-md focus:outline-none" aria-label="Toggle menu" onClick={() => setMenuOpen((s) => !s)}>
        ...
        {menuOpen && (
          <div className="fixed inset-x-0 top-[64px] bg-prime-5/95 backdrop-blur-sm md:hidden z-40 shadow-md">
            <div className="flex flex-col gap-4 p-4">
              <Link href="/"><button onClick={() => setMenuOpen(false)} className="text-left">Home</button></Link>
              <Link href="/about"><button onClick={() => setMenuOpen(false)} className="text-left">About</button></Link>
              <Link href="/contact"><button onClick={() => setMenuOpen(false)} className="text-left">Contact Us</button></Link>
              <Link href="/book"><button onClick={() => setMenuOpen(false)} className="text-left flex items-center justify-center text-cyan-300 font-semibold"><FaCalendarCheck className="inline mr-2" />Book Now</button></Link>

              {hasMounted && (
                <>
                  {!token ? (
                    <div className="flex gap-2 mt-2">
                      <Link href="/auth/login"><button onClick={() => setMenuOpen(false)} className="w-full flex items-center justify-center text-center px-4 py-2 bg-cyan-400/40 rounded-md text-white font-bold"><CiLogin className="inline mr-2" />Login</button></Link>
                      <Link href="/auth/signup"><button onClick={() => setMenuOpen(false)} className="w-full flex items-center justify-center px-4 py-2 bg-cyan-400/40 rounded-md text-white font-bold"><FaUserPlus className="inline mr-2" />Signup</button></Link>
                    </div>
                  ) : (
                    <div className="flex justify-between gap-2 mt-2">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <div>
                          ...
                          <button className="cursor-pointer text-sm text-cyan-200" onClick={()=>(window.location.href="/dashboard/profile"  )}>My Account</button>
                        </div>
                      </div>
                      {show && (
                        <div className="mt-2">
